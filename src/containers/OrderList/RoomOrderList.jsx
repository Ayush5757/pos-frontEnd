...
import { useNavigate } from "react-router-dom";
  ...
  const RoomOrderList = () => {
    ...
    useEffect(() => {
      const temp = localStorage.getItem("wathsappmessageRoom");
      if (temp) {
        setMessageData(() => temp);
    ...
    const { data, refetch } = useQuery({
      queryKey: ["getOrderListRoom"],
      queryFn: () => getOrderListRoom(selectDate, selectName, selectPhone,page),
      enabled: false,
    });
    ...
    const {
      ...
      queryKey: ["getOrderInformationRoom", localStorage.getItem("shop_user_id")],
      queryFn: () => getOrderInformationRoom(selectedOrderID),
      enabled: false,
    });
    ...
    const deleteOrders = useMutation({
      mutationFn: (newVal) => deleteOrderRoom(newVal),
    });
    ...
    return (
      <>
        <Box mt={20} p={"0 5px"}>
          ...
          >
            <Grid.Col span={12}>
              <PrimaryButton onClick={()=>navigate("/shop-order-list")}>Go to Orders</PrimaryButton>
           </Grid.Col>
            ...
            <Grid.Col span={12} xs={6} sm={6} md={4} lg={3}>
              <Formik
                ...
                onSubmit={(val) => {
                  ...
                  localStorage.setItem("wathsappmessageRoom", formattedMessage);
        ...
        <Box mt={50} className={classes.paginationBox}> 
        {/* {data && ( */}
          <Grid gutter={12}>
            <Grid.Col
              span={2}
          ...
          <Grid>
            <Grid.Col span={6}>
              ...
              <Box className={classes.infoBox}>
                <TextN1 className={classes.key}>
                  Room Price -
                  <span className={classes.value}>
                    {OrderSpecificInformation?.data?.target?.roomPrice ? `${OrderSpecificInformation?.data?.target?.roomPrice} ₹`:0} 
                  </span>
                </TextN1>
              </Box>
              <Box className={classes.infoBox}>
              ...
              <Box className={classes.infoBox}>
                <TextN1 className={classes.key}>
                  ...
                  <span className={classes.value} style={{ color: "orangered" }}>
                    {OrderSpecificInformation?.data?.target?.room?.room_name}
                  </span>
                </TextN1>
              </Box>
              <Box className={classes.infoBox}>
            ...
            <Grid.Col span={12}>
              ...
              {OrderSpecificInformation?.data?.target?.inventories?.map(
                (val, index) => (
                  <Grid mt={10}>
                    <Grid.Col span={1} key={index}>{`${index + 1} )`}</Grid.Col>
                    ...
                    <Grid.Col span={3}>
                    {val?.plate_type === "1" ? val?.qty * val?.price : val?.qty * val?.half_price} ₹
                   </Grid.Col>
                  </Grid>
                )
              )}
