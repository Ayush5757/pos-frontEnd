...
import {
  addToExistingroomOrder,
  getAlreadyOrderedroomItems,
  getIsroomBooked,
  getPendingOrderedroomItems,
  save_by_customer_order_inventrie_with_OTP,
  save_by_customer_order_room_inventrie,
  send_OTP_info_room_API,
} from "../../services/api/notificationroom";
import {
  AddCustomerInfoRoom,
  RemoveAllInventoriesRoom,
  RemoveCustomerOrderRoom,
} from "../../redux/slices/CustomerOrderRoom";
import { useNavigate } from "react-router-dom";
import { IconSoup } from "@tabler/icons-react";
import { IconReload } from "@tabler/icons-react";

const NotificationRoomContainer = () => {
  ...
  const { data, refetch } = useQuery({
    queryKey: ["isroomBooked", Cookies.get("customer_order_info")],
    queryFn: () =>
      getIsroomBooked({
        shopID: CustomerOrder?.shopID,
        roomID: CustomerOrder?.room?.roomID,
        roomName: CustomerOrder?.room?.room_name,
        roomTypeID: CustomerOrder?.room?.room_type_id,
      }),
    enabled: false,
  });
  ...
  const { data: already_Order_Item_data, refetch: alreadyOrderedIten_refetch } =
    useQuery({
      queryKey: [
        "already_Room_Order_Item_data",
        Cookies.get("customer_order_info"),
      ],
      queryFn: () =>
        getAlreadyOrderedroomItems({
          roomID: CustomerOrder?.room?.roomID,
  ...
  const add_to_existing_order = useMutation({
    mutationFn: (newVal) => addToExistingroomOrder(newVal),
  });
  ...
  const save_by_CustomerOrderInventorie = useMutation({
    mutationFn: (newVal) => save_by_customer_order_room_inventrie(newVal),
  });
  const send_OTP_info = useMutation({
    mutationFn: (newVal) => send_OTP_info_room_API(newVal),
  });
  ...
  return (
    <Box className={classes.big_box}>
      <Grid gutter={0} className={classes.big_grid}>
        <Grid.Col span={12} className={classes.notifyTopIcon}>
          <ActionIcon
            onClick={() => navigate("/customer-room-notification-order-notify")}
          >
        ...
        <Grid.Col span={12} className={classes.heading_page}>
          <TextN1 color={"#ffffff"}> Your Room Order's </TextN1>
        </Grid.Col>
            ...
            <Grid className={classes.heading} gutter={0}>
              <Grid.Col span={8} mb={3}>
                ...
                <TextN1 mt={5} color={"white"}>
                  Room -{" "}
                  <span className={classes.room_name}>
                    {CustomerOrder?.room?.room_name}
                  </span>
                </TextN1>
              </Grid.Col>
                        ...
                        <>
                          ...
                          >
                            <ActionIcon
                              ...
                              aria-label="Settings"
                              onClick={() => {
                                dispatch(RemoveCustomerOrderRoom(index));
                              }}
                            >
                              <IconTrash
                    ...
                    <>
                      {data?.data?.isBooked && (
                        <Grid.Col span={12}>
                          ...
                          <TextN1 className={classes.bookedMsg}>
                            OR You Can Scan Another Room Before Order...
                          </TextN1>
                        </Grid.Col>
                      )}
                      <Grid span={12}>
                                ...
                                try {
                                  if (!data?.data?.isBooked) {
                                    ...
                                    await send_OTP_info.mutateAsync(values, {
                                      onSuccess: () => {
                                        dispatch(AddCustomerInfoRoom(values));
                                        setisOTPFlag(true);
                                      },
                                    });
                                  }
                                  ...
                                  await save_by_CustomerOrderInventorie.mutateAsync(
                                    temp,
                                    {
                                      onSuccess: () => {
                                        ...
                                        dispatch(RemoveAllInventoriesRoom());
                                        shoeNotification(
                                          "success",
                                          "Your Order Placed"
                                        );
                  ...
                  await add_to_existing_order.mutateAsync(temp, {
                    onSuccess: (res) => {
                      if (res?.data?.isBooked) {
                        shoeNotification("error", "Room is Already Booked");
                      } else {
                        dispatch(RemoveAllInventoriesRoom());
                        alreadyOrderedIten_refetch();
                        pendingOrderedIten_refetch();
...
export default NotificationRoomContainer;
